...
from app.dependencies import session_dep, pagination_dep, PaginationParams, cur_user_dep
...
async def list_posts(db: session_dep, cur_user: cur_user_dep):
    return await get_all_posts(db)

@post_router.get('/user/{user_id}')
async def list_user_posts(user_id: int, db: session_dep, cur_user: cur_user_dep):
    return await get_user_posts(user_id, db)

@post_router.get('/{id}', status_code=status.HTTP_200_OK, response_model=PostGETDTO)
async def get_post(id: int, db: session_dep, cur_user: cur_user_dep):
    return await get_one_post(id, db)

@post_router.post('/', status_code=status.HTTP_201_CREATED)
async def new_post(post: PostPOSTDTO, db: session_dep, cur_user: cur_user_dep):
...
async def update_post(id: int, request: PostPOSTDTO, db: session_dep, cur_user: cur_user_dep):
