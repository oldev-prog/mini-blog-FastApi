...
async def user_comments(user_id: int, db: session_dep):
...
async def post_comments(post_id: int, db: session_dep):
    return await get_post_comments(post_id, db)

@comment_router.post('/', status_code=status.HTTP_201_CREATED)
async def create_comment(comment: CommentPOSTDTO, db: session_dep):
...
async def update_comment(id:int, data: CommentPOSTDTO, db: session_dep):
...
async def delete_comment(id:int, db: session_dep):
    await delete(id, db)
    return JSONResponse(
        {
