...
async def get_all_likes(db: session_dep, cur_user: cur_user_dep):
    return await get_likes(db=db)

@like_router.get('/user/{user_id}', status_code=status.HTTP_200_OK, response_model=List[LikeDTO])
async def get_user_likes(user_id: int, db: session_dep, cur_user: cur_user_dep):
...
async def get_post_likes(post_id: int, db: session_dep, cur_user: cur_user_dep):
    return await get_like_posts(post_id=post_id, db=db)

@like_router.post('/', status_code=status.HTTP_201_CREATED)
async def create_like(like: LikeDTO, db: session_dep, cur_user: cur_user_dep):
...
async def delete_like(user_id:int, post_id:int, db: session_dep, cur_user: cur_user_dep):
